...
/**
 * Team Report — start: <BK.Team Report><L>
 * processed => 710
 */
function _cp_process_team_report(PDO $conn, int $upload_id, string $files_dir) : void {
    echo '<div class="w3-card w3-padding w3-white w3-margin-top">';
    echo '<div class="w3-text-indigo"><b>Team Report</b></div>';
    ...
    if ($section === null) {
        echo '<div class="w3-panel w3-amber w3-border">Team Report section not found.</div></div>';
        return;
    }

    // TODO: parse & write to your tables.

    // Optional: set league/season/week
    [$league, $season, $week] = _cp_parse_lsw_from_filename($filename);
    _cp_mark_processed($conn, $upload_id, 710, $league, $season, $week);

    echo '<div class="w3-panel w3-pale-green w3-border">Team Report processed (code 710).</div>';
    echo '</div>';
}

/**
 * Roster — start: <BK.Roster><L>
 * processed => 720
 */
function _cp_process_roster(PDO $conn, int $upload_id, string $files_dir) : void {
    echo '<div class="w3-card w3-padding w3-white w3-margin-top">';
    echo '<div class="w3-text-indigo"><b>Roster</b></div>';
    ...
    if ($section === null) {
        echo '<div class="w3-panel w3-amber w3-border">Roster section not found.</div></div>';
        return;
    }
    ...
    [$league, $season, $week] = _cp_parse_lsw_from_filename($filename);
    _cp_mark_processed($conn, $upload_id, 720, $league, $season, $week);

    echo '<div class="w3-panel w3-pale-green w3-border">Roster processed (code 720).</div>';
    echo '</div>';
}

/**
 * Play by Play — start: <ST.118><C><B>1st Quarter<L.54.1>
 * (Often spans multiple quarters until <E><P>)
 * processed => 720  (as you specified)
 */
function _cp_process_play_by_play(PDO $conn, int $upload_id, string $files_dir) : void {
    echo '<div class="w3-card w3-padding w3-white w3-margin-top">';
    echo '<div class="w3-text-indigo"><b>Play by Play</b></div>';
    ...
    // TODO: parse PBP events; insert into your pbp table.

    [$league, $season, $week] = _cp_parse_lsw_from_filename($filename);
    _cp_mark_processed($conn, $upload_id, 720, $league, $season, $week);

    echo '<div class="w3-panel w3-pale-green w3-border">Play by Play processed (code 720).</div>';
    echo '</div>';
}

/**
 * Head to Head — start: <E><ST.66><L>
 * processed => 730
 */
function _cp_process_head_to_head(PDO $conn, int $upload_id, string $files_dir) : void {
    echo '<div class="w3-card w3-padding w3-white w3-margin-top">';
    echo '<div class="w3-text-indigo"><b>Head to Head</b></div>';

    [$urow, $filename, $raw] = _cp_load_upload_and_file($conn, $upload_id, $files_dir);
    if ($raw === null) { echo '</div>'; return; }

    // Per your note, the start string includes the <E> preceding it
    $section = _cp_extract_section($raw, '<E><ST.66><L>');
    if ($section === null) {
        echo '<div class="w3-panel w3-amber w3-border">Head to Head section not found.</div></div>';
        return;
    }

    // TODO: parse H2H summary and write to DB.

    [$league, $season, $week] = _cp_parse_lsw_from_filename($filename);
    _cp_mark_processed($conn, $upload_id, 730, $league, $season, $week);

    echo '<div class="w3-panel w3-pale-green w3-border">Head to Head processed (code 730).</div>';
    echo '</div>';
}

/**
 * Standings — start: <B><BK.Standings><L>
 * processed => 740
 */
function _cp_process_standings(PDO $conn, int $upload_id, string $files_dir) : void {
    echo '<div class="w3-card w3-padding w3-white w3-margin-top">';
    echo '<div class="w3-text-indigo"><b>Standings</b></div>';
    ...
    if ($section === null) {
        echo '<div class="w3-panel w3-amber w3-border">Standings section not found.</div></div>';
        return;
    }

    // TODO: parse standings table(s) and write to DB.

    [$league, $season, $week] = _cp_parse_lsw_from_filename($filename);
    _cp_mark_processed($conn, $upload_id, 740, $league, $season, $week);

    echo '<div class="w3-panel w3-pale-green w3-border">Standings processed (code 740).</div>';
    echo '</div>';
}

/**
 * Scouting Report — start: <BK.Scouting Report><L>
 * processed => 750
 */
function _cp_process_scouting_report(PDO $conn, int $upload_id, string $files_dir) : void {
    echo '<div class="w3-card w3-padding w3-white w3-margin-top">';
    echo '<div class="w3-text-indigo"><b>Scouting Report</b></div>';
    ...
    if ($section === null) {
        echo '<div class="w3-panel w3-amber w3-border">Scouting Report section not found.</div></div>';
        return;
    }

    // TODO: parse scouting content and write to DB.

    [$league, $season, $week] = _cp_parse_lsw_from_filename($filename);
    _cp_mark_processed($conn, $upload_id, 750, $league, $season, $week);

    echo '<div class="w3-panel w3-pale-green w3-border">Scouting Report processed (code 750).</div>';
    echo '</div>';
}
