...
echo "<h1>Gameplan College Football Results Page</h1>";
...
#Show League Selector
	...
	//Start the form
	echo "<form action='index.php?function=show_static_page&id_static_page=33' method='post'>\n";	
...
echo "<h3>Week $_cp_mydispweek Results</h3>";
	//Display week choice
	#Show League Selector
	$_cp_sql65 = "SELECT DISTINCT `week` FROM `vg_collegegames` WHERE `league` LIKE '$_cp_myleague' AND `season` LIKE '$_cp_myseason' ORDER BY `week`  DESC";        
...
if ($mymobile==1) {
$_cp_sql55="SELECT c.`city` AS `Team2`, CONCAT(`opp_score`, ' - ',`score`) as `myscore`,  b.`city` AS `Team1` 
FROM `f_games` a 
...
//Loop through results
echo "<div class='$mycolour2 w3-center' style='width:100% '>$row56[1]</caption>";
...
if ($number_of_rows55>0) {
//Loop through results
echo "<div class='$mycolour2 w3-center' style='width:60% '>$row56[1]</caption>";
while ($row = fetch_row_db($result55)) {
$team1=str_pad($row[0],25," ");
$score=str_pad($row[1],25," ");
$team2=str_pad($row[2],25," ");
