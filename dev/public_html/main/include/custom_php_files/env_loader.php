...
final class Env {
    ...
    public static function int(string $k, int $d=0): int { $v=self::get($k,$d); return is_numeric($v)?(int)$v:$d; }
    public static function bool(string $k, bool $d=false): bool {
        $v=strtolower((string)self::get($k,$d?'1':'0')); return in_array($v,['1','true','yes','on'],true);
    }
}
